...
sub test_includepath : Local {
    ...
    if ( $c->request->param('addpath') ){
        ...
        my $view = 'TestApp::View::Haml::' . ($c->request->param('view') || $c->config->{default_view});
        no strict "refs";
...
sub test_render : Local {
    ...
    $c->stash->{message} = eval { $c->view('Haml::Appconfig')->render($c, $c->req->param('template'), {param => $c->req->param('param') || ''}) };
    if (my $err = $@) {
        $c->response->body($err);
        $c->response->status(403);
    } else {
        $c->stash->{haml} = 'test.haml';
    }
...
sub test_msg : Local {
    ...
    $c->stash->{message} = $c->view('Haml::AppConfig')->render($c, \$tmpl);
    $c->stash->{haml} = 'test.haml';
}
